...
IR::Program TranslateProgram(const std::span<const u32>& code, Pools& pools, Info& info,
    ...
    { static const int pshift = [] { const char* const e = std::getenv("SHADPS4_PARAM_SHIFT"); return e != nullptr ? std::atoi(e) : 0; }(); static const u64 sh = [] { const char* const e = std::getenv("SHADPS4_SHIFT_HASH"); return e != nullptr ? std::strtoull(e, nullptr, 16) : 0ULL; }(); static const bool shalf = [] { const char* const e = std::getenv("SHADPS4_SHIFT_HASH"); return e != nullptr && std::strlen(e) <= 8; }(); const bool smatch = sh == 0 || (shalf ? (info.pgm_hash >> 32) == sh : info.pgm_hash == sh); if (pshift != 0 && smatch) { u32 n = 0; for (IR::Block* const b : program.blocks) { for (IR::Inst& i : *b) { if (i.GetOpcode() != IR::Opcode::GetAttribute) { continue; } const IR::Attribute a = i.Arg(0).Attribute(); if (!IR::IsParam(a)) { continue; } const int k = static_cast<int>(u32(a) - u32(IR::Attribute::Param0)) + pshift; if (k >= 0 && k < 32) { i.SetArg(0, IR::Value{static_cast<IR::Attribute>(u32(IR::Attribute::Param0) + static_cast<u32>(k))}); ++n; } } } LOG_INFO(Render_Recompiler, "decalage {} applique a {} lectures, shader {:#x}", pshift, n, info.pgm_hash); } }
